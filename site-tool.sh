...
ssl_proxy_part() {
  local upstream="$1"
  local upstream_host upstream_scheme
...
add_emby_proxy() {
  ...
  local upstream_host conf_file ssl_dir backup
  upstream_host="$(get_host_from_url "$upstream")"
...
add_emby_split_proxy() {
  ...
  local main_host stream_host conf_file ssl_dir backup
...
add_normal_proxy() {
  ...
  local upstream_host conf_file ssl_dir host_header backup
  upstream_host="$(get_host_from_url "$upstream")"
  conf_file="${CONF_DIR}/${domain}.conf"
  ssl_dir="${SSL_BASE}/${domain}"
  echo
...
add_static_site() {
  ...
  local conf_file ssl_dir backup
  conf_file="${CONF_DIR}/${domain}.conf"
  ssl_dir="${SSL_BASE}/${domain}"
  echo
  echo "网站域名: $domain"
...
scan_existing_configs() {
  ...
  for conf in "$CONF_DIR"/*.conf; do
    ...
    local domains
    domains="$(grep -E '^\s*server_name\s+' "$conf" \
    ...
    for domain in $domains; do
      ...
      if grep -q "fastcgi_pass" "$conf"; then
        ...
        backend="$(grep -E '^\s*proxy_pass\s+' "$conf" | head -2 \
                  | sed -E 's/^\s*proxy_pass\s+//;s/;//' | paste -sd ',' -)"
...
add_site_menu() {
  while true; do
    ...
    echo "3. 普通网站反代"
    echo "4. 静态网站"
    echo "5. PHP 网站 (Typecho/WordPress)"
    echo "0. 返回"
    echo
    read -rp "请选择: " choice
    case "$choice" in
      ...
      3) add_normal_proxy;      pause ;;
      4) add_static_site;       pause ;;
      5) add_php_site;          pause ;;
      0) return ;;
      *) echo "无效选择"; sleep 1 ;;
    esac
  done
...
config_menu() {
  while true; do
    ...
    read -rp "请选择: " choice
    case "$choice" in
      1) scan_existing_configs;   pause ;;
      2) list_sites;              pause ;;
      3) delete_site_config;      pause ;;
      4) delete_site_config yes;  pause ;;
      5) delete_cert_only;        pause ;;
      0) return ;;
      *) echo "无效选择"; sleep 1 ;;
    esac
  done
...
main_menu() {
  while true; do
    clear
    echo "========================="
    echo " site-tool 站点管理工具"
    echo "========================="
    echo "1. 添加站点"
    echo "2. 配置管理 (扫描/查看/删除)"
    echo "3. 证书管理"
    echo "0. 退出"
    echo
    read -rp "请选择: " choice
